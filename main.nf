...
def versionMessage(){
    log.info"""
        ========================================================================
                        ASSEMBLY WORKFLOW - version ${version}
        ========================================================================
    """.stripIndent()
}
...
def helpMessage() {
    ...
    Either 'indir' and 'fastq_pattern' must be specified if using local short reads,
    or 'accession_number_file' if specifying samples in the SRA for which the fastqs will be downloaded.
        --indir                         Path to input directory containing the fastq files to be assembled
...
Helper.check_optional_parameters(params, ['indir', 'accession_number_file'])

// set up output directory
outdir = Helper.check_mandatory_parameter(params, 'outdir') - ~/\/$/
// check that the user has specified an outdir that differs from the default './.default_output' required for --help to work
if (outdir =~ /.default_output/) {
    println "You must specify an output directory with --outdir"
    System.exit(1)
}
...
log.info ""
log.info "======================================================================"
log.info "                  GHRU ASSEMBLY WORKFLOW"
log.info "======================================================================"
...
if ( prescreen_genome_size_check ) {
    ...
    process write_out_excluded_genomes {
        ...
        publishDir "${outdir}/estimated_size_of_excluded_genomes"
        input:
...
process qc_pre_trimming {
    ...
    if (full_output){
        publishDir "${outdir}/fastqc/pre_trimming",
        mode: 'copy',
        pattern: "*.html"
    }
...
process qc_post_trimming {
    ...
    publishDir "${outdir}/fastqc/post_trimming",
        mode: 'copy',
        pattern: "*.html"
...
process fastqc_multiqc {
    ...
    publishDir "${outdir}/quality_reports",
        mode: 'copy',
...
process species_identification {
    tag{pair_id}
    publishDir "${outdir}/bactinspector", mode: 'copy'  // JWP add
    ...
    mv species_investigation*.tsv species_investigation_${pair_id}.tsv # JWP added
    """
}
...
process genome_size_estimation {
    tag { pair_id }
    publishDir "${outdir}/mash_genome_size", mode: 'copy'  // JWP add

    input:
    set pair_id, file(file_pair)  from trimmed_fastqs_for_genome_size_estimation

    output:
    set pair_id, file('*mash_stats.out') into mash_output // JWP ed

    // JWP: I removed the failing `kat` command in `genome_size estimation` and replaced
    // the resulting 'minima' variable with a hardcoded '3', as per Bactopia in
    // https://github.com/bactopia/bactopia/blob/master/modules/local/bactopia/gather_samples/main.nf
    // Old code:
    // kat hist --mer_len 21  --thread 1 --output_prefix ${pair_id} ${file_pair[0]}
    // minima=`cat  ${pair_id}.dist_analysis.json | jq '.global_minima .freq' | tr -d '\\n'`
    """
    minima=3 # JWP ed
    mash sketch -o sketch_${pair_id} -k 32 -m \$minima -r ${file_pair[0]} 2> ${pair_id}_mash_stats.out # JWP ed
    """
}

// channel to output genome size from mash output
mash_output
    .map { pair_id, file -> find_genome_size(pair_id, file.text) }
    .into{ genome_size_estimation_for_read_correction; genome_size_estimation_for_downsampling }

trimmed_fastqs_and_genome_size = trimmed_fastqs_for_correction
    .join(genome_size_estimation_for_read_correction)
...
process read_correction {
    ...
    for file in corrected_fastqs/*.cor.fq.gz; do
...
process check_for_contamination {
    ...
    // Removed -d argument, databases not needed for Salmonella, see https://github.com/OLC-Bioinformatics/ConFindr
    ...
    if (file_pair[0] =~ /_R1/) { // files with _R1 and _R2
        """
        confindr.py -i . -o . -t 2 -bf 0.025 -b 2 --cross_detail -Xmx 1500m # JWP ed
        """
    } else { // files with _1 and _2
        """
        confindr.py -i . -o . -t 2 -bf 0.025 -b 2 --cross_detail -Xmx 1500m -fid _1 -rid _2 # JWP ed
        """  
    }
}
...
corrected_fastqs_and_genome_size_and_base_count = corrected_fastqs_for_merging
    .join(genome_size_estimation_for_downsampling)
    .join(base_counts)
...
process spades_assembly {
    ...
    if (careful) careful_arg = "--careful" else careful_arg = ""
    
    """
    mem_int=`echo ${task.memory} | sed 's/ .*//'`

    spades.py \
        --pe1-1 ${file_triplet[1]} \
        --pe1-2 ${file_triplet[2]} \
        --pe1-m ${file_triplet[0]} \
        --only-assembler \
        ${careful_arg} \
        -o . \
        --tmp-dir /tmp/${pair_id}_assembly \
        -k ${kmers} \
        --threads ${task.cpus} \
        --memory \$mem_int
    """
...
if (params.qc_conditions) {
    ...
    publishDir "${outdir}/assemblies/pass",
        mode: 'copy',
        pattern: 'assemblies/pass/*',
        saveAs: { file -> file.split('\\/')[-1] }

    publishDir "${outdir}/assemblies/warning",
        mode: 'copy',
        pattern: 'assemblies/warning/*',
        saveAs: { file -> file.split('\\/')[-1] }
    
    publishDir "${outdir}/assemblies/failure",
        mode: 'copy',
...
process qualifyr_report {
    ...
    publishDir "${outdir}/quality_reports", mode: 'copy', pattern: "qualifyr_report.*"
    ...
    qualifyr report \
        -i . \
        -c 'quast.N50,quast.# contigs (>= 1000 bp),quast.Total length (>= 1000 bp),confindr.contam_status,bactinspector.species' \
        -s "Analysis with GHRU Assembly Pipeline version ${version}<br><br>Command line:<br>${workflow_command}"
    """
}
} else {
