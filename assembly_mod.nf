...
process trimming {
  ...
  input:
  val(min_read_length) from min_read_length
  ...
  trimmomatic PE -phred33 ${file_pair[0]} ${file_pair[1]} trimmed_fastqs/${file_pair[0]} /dev/null trimmed_fastqs/${file_pair[1]} /dev/null ILLUMINACLIP:${adapter_file}:2:30:10 SLIDINGWINDOW:4:20 LEADING:25 TRAILING:25 MINLEN:${min_read_length}  
  """
}
...
process spades_assembly {
  ...
  """
  spades.py --pe1-1 ${file_triplet[1]} --pe1-2 ${file_triplet[2]} --pe1-m ${file_triplet[0]} --only-assembler  -o . --tmp-dir /tmp/${pair_id}_assembly -k 21,33,43,53,63,75 --memory 4 --threads 1
  """
...
process filter_scaffolds {
  ...
  output:
  set pair_id, file("${pair_id}_scaffolds.fasta") into scaffolds_for_single_analysis, scaffolds_for_combined_analysis
...
process quast_summary {
  ...
  input:
  file(contig_files) from scaffolds_for_combined_analysis.collect { it[1] }
